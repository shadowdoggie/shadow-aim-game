...
- Juniper's personal notes about facts and preferences you share. These are stored separately for each signed-in account on this computer; they do not sync between computers.

## What AI features send

Coaching sends measured results, aggregate evidence, relevant earlier rounds and advice, and questions you ask. When voice is connected, your microphone audio and current training context are sent to GPT-Live. Game context includes settings, current drill, provisional performance measurements, visible controls, and selected personal notes for the current account. Song searches and playback requests may send song titles, artists, albums, and opaque track identifiers so Juniper can identify your selection. Raw local music paths are not needed by the voice model.
...
Ask Juniper to remember, correct, or forget a personal note. Settings shows saved personal notes and lets you clear them for the current account. Clearing these notes does not erase round history or data already sent to OpenAI. Notes are limited to short facts and preferences, rather than full conversation transcripts; not every past conversation will be recalled. Account switching removes the previous account's personal notes from live context.
