...
class TestMusic:
	...
	func _prepare_track(_id: String) -> Dictionary:
...
func run() -> void:
	...
	assert(response.ok and music.is_playing())
	...
	print("MUSIC_UI_SMOKE_PASS: native WAV loading, search UI, no autoplay, dedicated volume and mute, pause/resume/stop/next, cancelled prepare, expired voice request, failure preserves song, cleanup")
	quit()
